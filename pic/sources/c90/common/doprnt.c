...
#if	(__SIZEOF_LONG__ == __SIZEOF_INT__ || defined(__FLOAT)) && !defined(__LONG)
#define	__LONG	1
#endif
...
#elif	defined(_PSOC_)
#undef	frexp
#define	frexp(val, ptr) (void)(*(ptr) = ((unsigned char)(*(__uint24 *)&val >> 15)) - 126)
#elif	__SIZEOF_DOUBLE__ == 3 && DBL_MAX_EXP == 128
#undef	frexp
#define	frexp(val, ptr) (void)(*(ptr) = ((*(unsigned long *)&val >> 15) & 255) - 126)
#endif

#if	defined(__FLOAT) && __SIZEOF_DOUBLE__ == 8
#define	DOUBLE	1
...
#define	expon	int
#define	uns_expon	unsigned int
#else
#define	expon	signed char
#define	uns_expon	unsigned char
#endif
...
scale(expon scl)
{
	...
	if(scl < 0) {
		scl = -scl;
		if(scl>=110)
			return _npowers_[(uns_expon)(scl/100+18)] * _npowers_[(uns_expon)((scl%100)/10+9)] * _npowers_[(uns_expon)(scl%10)];
		else if(scl > 10)
			return _npowers_[(uns_expon)(scl/10+9)] * _npowers_[(uns_expon)(scl%10)];
		return _npowers_[(uns_expon)scl];
	}
	if(scl>=110)
		return _powers_[(uns_expon)(scl/100+18)] * _powers_[(uns_expon)((scl%100)/10+9)] * _powers_[(uns_expon)(scl%10)];
	else if(scl > 10)
		return _powers_[(uns_expon)(scl/10+9)] * _powers_[(uns_expon)(scl%10)];
	return _powers_[(uns_expon)scl];
}
...
#define	pputc(c)	do { \
						if(pb->func) \
							(pb->func((char)(c)))INCR_CNT; \
						else \
							((*pb->ptr++ = (char)(c))INCR_CNT); \
					} while(0)
...
#ifdef	__LONG
	int		prec;
#define uns_prec	unsigned int
#define prec_t		int
#else
	signed char	prec;
#define uns_prec	unsigned char
#define prec_t		signed char
#endif
	FLAG_SIZE	flag;
#ifdef	RETVALUE
	int		ccnt = 0;
...
dostring:
		...
		if(flag & (FLOATFORMAT)) {
			...
			eexp = 0;		/* If the number is zero, the exponent is zero. */
			if( fval!=0) {		/* If the number is non-zero, find the exponent. */
				frexp(fval, &eexp);		/* get binary exponent */
				eexp--;				/* adjust 0.5 -> 1.0 */
				eexp *= 3;
				eexp /= 10;			/* estimate decimal exponent */
				if(eexp < 0)
					eexp--;
				...
				//integ = fval * scale(-eexp);
				tmpval.integ = scale(-eexp);		// replaced with this
				tmpval.integ *= fval;
				if(tmpval.integ < 1.0)
					eexp--;
			...
			if(eexp <= 0)	// value is 0.??? (neg expnt) whole characters = 1 (allow zero)
				c = 1;
			else
				c = (char)eexp;
			if(
...
#endif
#ifdef	GFMT
					(flag & GFMT && (eexp < -4 || eexp >= (int)prec))
#endif
				) {	/* use e format */
#ifdef	GFMT
				if(prec && flag & GFMT)
				...
				if( fval!=0) {	/* Normalise only if the number is non-zero. */
					...
					if(fval - (double)(unsigned long)fval >= 0.5)
						fval += 0.5;
					if((unsigned long)fval >= fdpowers[c+1u]) {
						fval *= 1e-1;
						eexp++;
				...
				if(eexp >= 1000 || eexp <= -1000)	/* 4 digit exponent */
					width--;
#endif
				if(eexp >= 100 || eexp <= -100)	/* 3 digit exponent */
					width--;
#endif
#endif	// WIDTH
#ifdef	FILL
...
#ifdef	GFMT
				if(flag & GFMT) {
					if(eexp < 0)	// fractional part only
						prec -= eexp-1;
					ival = _dto64i(fval);
					for(c = 1 ; c != NDDIG ; c++)
						if(ival < fdpowers[c])
...
#endif	// GFMT
				if(prec <= (int)NDDIG)
					fval += fround((uns_prec)prec);
				if(eexp > (int)(NDDIG)-2) {
					eexp -= NDDIG-2;
					ival = _divto64i(fval, scale(eexp));
					fval = 0.0;
				} else {
					ival = _dto64i(fval);
					fval -= _64itod(ival);
				...
				if((eexp > 9)||(fval != 0 && (unsigned long)fval == 0 && eexp > 1)) {
					// fval is > 4.2e9
					// new eexp must be such that div_to_l() is < 4.2e9
					// OLD CODE:	if(fval / scale(eexp) < 4.294967296){
			...
			while(c)
				pputc('0' + _mod64i10(_div64(ival, fdpowers[--c])));
#else	// DOUBLE
			val = (unsigned value)(fval * scale((expon)c));
			while(c--) {
				tmpval.vd = val/dpowers[c];
				tmpval.vd %= 10;
				pputc('0' + tmpval.vd);
...
#ifdef	LONG
			if(flag & LONG)
				val = (unsigned value)va_arg(ap, long);
			else
#endif	// LONG
				val = (unsigned value)va_arg(ap, int);
#ifdef	NEGSIGN
			if((value)val < 0) {
				flag |= NEGSIGN;
				val = -val;
...
#if	defined(EIGHT) || defined(SIXTEEN) || defined(UNSIGN)
		{
			...
			if(flag & POINTER)
				val = (unsigned long)va_arg(ap, __far char *);
			else
#endif
#ifdef	LONG
				if(flag & LONG)
		...
		if(flag & FILL) {
			...
			if(flag & SPCSIGN)
				pputc(' ');
#endif
#if defined(ALTERN) && defined(SIXTEEN) && defined(BASEM)
		...
		case UNSIGN:
#endif
#endif	// MULTRAD
				c = (val / dpowers[(uns_prec)prec]) % 10 + '0';
#ifdef	MULTRAD
				break;
#endif
#endif	// TEN || UNSIGN
...
#endif
			{
				unsigned char idx = (val / hexpowers[(uns_prec)prec]) & 0xF;
			...
			case EIGHT:
#endif	// MULTRAD
				c = ((val / octpowers[(uns_prec)prec]) & 07) + '0';
#ifdef	MULTRAD
				break;
#endif	// MULTRAD
#endif	// EIGHT
